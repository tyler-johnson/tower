//! `atc serve` — the standing process, in the foreground.
//!
//! The verb's own work is small: resolve the address and the port, hand
//! them and the repository to `atc-serve`, say where it landed, and
...
fn resolve(
    ...
    let host = match loud(flag_host, "--host", "ATC_HOST") {
    ...
    let port = match loud(flag_port, "--port", "ATC_PORT") {
...
fn parse_host(raw: &str, lane: &str) -> Result<IpAddr, CliError> {
    config::parse_host(raw).ok_or_else(|| {
        CliError::coded(
            ...
            vec!["atc serve --host <addr>".to_string()],
        )
    })
}
...
fn parse_port(raw: &str, lane: &str) -> Result<u16, CliError> {
    config::parse_port(raw).ok_or_else(|| {
        CliError::coded(
            ...
            vec!["atc serve --port <n>".to_string()],
        )
    })
}
