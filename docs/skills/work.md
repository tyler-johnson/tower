...
The loop step is `atc next --json` — your own queue, where a held flight answered back to Ready lands now that a pull is yours, your client's lane, the shared pool, then the unassigned lane. Exit 0 is a pick: `picked[0]` carries `id`, `writer`, `display`, `subject`, and — when the flight's part names one — `skill`. Use `id` in commands and `display` when reporting to a person. Exit 1 is an empty pick, and `data.outcome` says which: `drained` is a board with nothing left, and `elsewhere` is work that exists in another lane — someone's own queue. Both stop the loop and are reported by their word. Those are the only exits. Never sleep and retry, never add a timeout, never invent a sentinel.

Your callsign is `ATC_CALLSIGN` when the launcher set one, else the word you gave `atc callsign` this session, else the client you run under, else your login name. When your brief names you, run `atc callsign <name>` first thing; otherwise fly as your client. Subagents share the session and its callsign, so their events carry yours; work to be tracked as a separate pilot is a separate session — `claude -p` under `ATC_SESSION` and `ATC_CALLSIGN` — and a launcher that runs a pool sets `ATC_SESSION` per worker.

Fan-out: when the harness can run parallel subagents, `atc next -n 3` claims the next three by priority, then filed order. Hand each picked row to one worker in its own worktree, and rejoin the loop when all of them have ended their flight with a verb. Solo remains the default; fan out only when the board shows independent ready flights and the harness genuinely runs workers concurrently.
