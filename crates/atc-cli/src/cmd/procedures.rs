//! `atc procedures [<name>]` — what is installed, and where it
//! came from.
//!
...
use atc_core::procedure::{self, Definition, Registry};
...
fn list(installed: &Registry, repo_root: Option<&Path>, colored: bool) -> String {
    ...
    out.push_str(&render::paint_dim(
        &format!("{} {noun} · atc procedures <name> for one", installed.len()),
        colored,
    ));
    out.push('\n');
    out
