...
| `atc briefing` | one line for a session-start hook — the Ready count, pointing at `atc` when there is something to pull; the hook runs it under a short box and drops anything longer than one line | hook |
