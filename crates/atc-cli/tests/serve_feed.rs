...
fn file(repo: &Path, subject: &str) {
    stdout(&support::atc(repo, &["file", subject, "--json"]));
}
...
fn the_feed_opens_with_the_current_board() {
    ...
    matches_cli(&event, &stdout(&support::atc(repo.path(), &["--json"])));
}
...
fn filed_kind(subject: &str, status: &str) -> atc_core::log::Kind {
    atc_core::log::Kind::Filed {
        procedure: None,
...
fn the_server_survives_its_watch_child_dying() {
    ...
    let server = Server::start(
        ...
        &[("ATC_FF", &fake.path().to_string_lossy())],
    );
