<script lang="ts">
	...
	let step = $derived(
		b && brief ? neighbors(b, brief.id) : { prev: null, next: null }
	);
</script>
...
<main class="mx-auto flex max-w-7xl flex-col gap-6 p-4">
	<div class="flex items-center gap-3">
		<nav class="breadcrumbs flex-1 text-sm text-base-content/60">
			<ul>
				...
				<li><span class="font-mono text-primary">{ref}</span></li>
			</ul>
		</nav>
		<div class="join">
			<a
	...
	{:else}
		<p class="p-4 text-sm text-base-content/60">reading the brief…</p>
	{/if}
</main>
