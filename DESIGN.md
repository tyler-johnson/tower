...
| `atc next [<lane>...] [--assignee <lane>] [-n <k>] [--peek]` | claim the next Ready flight from the lanes named in order — your own queue when unsaid, then the unassigned overflow — or `k` of them in filed order; the pick sets In Progress under your callsign and moves the flight into your queue unless `--assignee` says where; `--peek` is the same computation with nothing written | an agent |
