...
tower is a declared extension of fufu's, and three flags answer fufu's handshakes ahead of any verb: `--ff-manifest` says what tower is — its verbs, that nothing it writes is undoable, that it offers a briefing line and tools; `--ff-tools` describes every verb an agent has a reason to call as a typed tool, generated from the command line's own grammar, so `ff mcp` serves them as `tower__<verb>`; and `--ff-skill <name>` hands `ff hook` the files behind a skill the manifest lists. All three are answered from what is compiled in, outside any repository, because that is where `ff extension add tower` asks.
