//! `atc edit` against real repositories: the overlay on a flight's
...
fn repo_with_a_comment() -> (Repo, String) {
    let repo = repo();
    stdout(&atc(repo.path(), &["file", "the flight", "-m", "the body"]));
    stdout(&atc(
        repo.path(),
...
fn a_subject_edit_rewords_the_flight_and_echoes() {
    let repo = repo();
    stdout(&atc(repo.path(), &["file", "the wrogn subject"]));

    let out = stdout(&atc(repo.path(), &["edit", "1", "-s", "the right subject"]));
    ...
    let board = stdout(&atc(repo.path(), &["board"]));
...
fn a_body_edit_rewords_the_body_alone() {
    ...
    stdout(&atc(repo.path(), &["edit", "1", "-m", "the new body"]));
...
fn a_comment_edits_by_its_printed_id() {
    ...
    assert_eq!(out, format!("edited comment {comment} on #1\nboard: atc\n"));
...
fn a_done_flight_still_edits() {
    ...
    // motivating case.
    let repo = repo();
    stdout(&atc(repo.path(), &["file", "finished wrogn"]));
    stdout(&atc(repo.path(), &["done", "1"]));

    stdout(&atc(repo.path(), &["edit", "1", "-s", "finished right"]));
    let text = stdout(&atc(repo.path(), &["brief", "1"]));
...
fn no_flags_refuses_needs_edit() {
    let repo = repo();
    stdout(&atc(repo.path(), &["file", "the flight"]));
    let out = atc(repo.path(), &["edit", "1", "--json"]);
...
fn an_empty_subject_refuses() {
    let repo = repo();
    stdout(&atc(repo.path(), &["file", "the flight"]));
    let out = atc(repo.path(), &["edit", "1", "-s", "   ", "--json"]);
...
fn a_malformed_reference_refuses_bad_flight() {
    let repo = repo();
    let out = atc(repo.path(), &["edit", "not-an-id", "-s", "s", "--json"]);
...
fn an_id_naming_nothing_refuses_not_found() {
    let repo = repo();
    stdout(&atc(repo.path(), &["file", "the flight"]));
    let out = atc(repo.path(), &["edit", "pi.99", "-s", "s", "--json"]);
