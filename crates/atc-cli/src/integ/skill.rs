...
pub fn write_all(root: &Path) -> Result<(), CliError> {
    super::adapter_skills::write(root)?;
...
pub fn remove_all(root: &Path) -> Result<bool, CliError> {
    let mut removed = super::adapter_skills::remove(root)?;
...
pub fn wiring(root: &Path) -> Wiring {
    ...
    if present == 0 && stale.is_empty() {
        Wiring::NotWired
    } else if current == SKILLS.len() && stale.is_empty() && super::adapter_skills::current(root) {
        Wiring::Wired {
