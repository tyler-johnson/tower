...
| `atc session [--mint]` | every session on this machine with a lease — client, callsign, fresh or stale, pid — yours marked; `--mint` prints a fresh id and touches nothing | either |
...
| `atc trigger` | what a wired client runs on every event it offers, dispatched on the payload's event name in three classes: a context boundary prints the notice — the flights In Progress under your callsign, pointing at `atc brief`, else the Ready count, pointing at `atc` when there is something to pull — and renews the session's lease; activity renews the lease and prints nothing; the session's end releases it. The `shell` source is a terminal's rc lines: no payload, a heartbeat before every prompt, `--end` the release at exit, and never a notice. The hook runs it under a short box, and it exits 0 saying nothing whatever went wrong | hook |
...
- **The trigger.** A wired client runs `atc trigger` on every event it offers: at a context boundary the notice, one line, a short box, dropped whole past either; on activity a heartbeat on the session's lease; at the end its release.
