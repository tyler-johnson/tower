//! Self-update: the explicit `atc update` lane plus the passive
...
/// Release builds set ATC_OFFICIAL_BUILD in CI; everything else — dev,
...
pub fn asset_name_for(version: &str, os: &str, arch: &str) -> Option<String> {
    ...
    Some(format!("atc_{version}_{os_map}_{arch_map}.{ext}"))
}
...
impl Updater {
    pub fn run(&self) -> Result<Outcome, CliError> {
        ...
        match classify_install(&self.exe, self.official) {
            InstallKind::Source => {
                return Err(CliError::coded(
                    "update/source-build",
                    "atc was built from source — update with: cargo install --git https://github.com/tyler-johnson/tower atc-cli",
                    vec![
            ...
            InstallKind::Homebrew => {
                return Err(CliError::coded(
                    "update/homebrew",
                    "atc was installed with Homebrew — update with: brew upgrade atc",
                    vec!["brew upgrade atc".to_string()],
                ));
            }
            InstallKind::Official => {}
        }
        ...
        let member = if cfg!(windows) { "atc.exe" } else { "atc" };
...
mod tests {
    ...
    fn asset_name_for_platforms() {
        assert_eq!(
            ...
            Some("atc_0.2.0_linux_amd64.tar.gz".into()),
        );
        assert_eq!(
            asset_name_for("0.2.0", "macos", "aarch64"),
            Some("atc_0.2.0_darwin_arm64.tar.gz".into()),
        );
        assert_eq!(
            asset_name_for("0.2.0", "windows", "aarch64"),
            Some("atc_0.2.0_windows_arm64.zip".into()),
        );
    ...
    fn checksum_for_matching_and_missing() {
        let fixture = "ABC123  atc_0.1.0_linux_amd64.tar.gz\njunk extra line here\nDEF456  atc_0.1.0_darwin_arm64.tar.gz";
        assert_eq!(
            checksum_for(fixture, "atc_0.1.0_linux_amd64.tar.gz"),
            Some("abc123".into()),
        );
        assert!(checksum_for(fixture, "atc_missing.tar.gz").is_none());
    }

    #[test]
    fn classify_install_kinds() {
        assert_eq!(
            classify_install(std::path::Path::new("/usr/local/bin/atc"), false),
            InstallKind::Source,
        );
        assert_eq!(
            classify_install(std::path::Path::new("/opt/homebrew/bin/atc"), true),
            InstallKind::Homebrew,
        );
        assert_eq!(
            classify_install(
                std::path::Path::new("/home/linuxbrew/.linuxbrew/bin/atc"),
                true
            ),
            InstallKind::Homebrew,
        );
        assert_eq!(
            classify_install(
                std::path::Path::new("/usr/local/Cellar/atc/0.1.0/bin/atc"),
                true
            ),
            InstallKind::Homebrew,
        );
        assert_eq!(
            classify_install(std::path::Path::new("/usr/local/bin/atc"), true),
            InstallKind::Official,
        );
    }
}
...
mod http_tests {
    ...
    /// Build a tar.gz archive containing a `atc` member with the given
    ...
    fn build_archive(bin_contents: &[u8]) -> (Vec<u8>, String) {
        let mut tar_data = Vec::new();
        {
            ...
            builder
                .append_data(&mut header, "atc", bin_contents)
                .unwrap();
    ...
    fn happy_path() {
        ...
        let exe_path = dir.path().join("atc");
