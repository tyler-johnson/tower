//! `atc serve` against the real binary: the two lanes and their four
...
//! `ATC_HOST` and `ATC_PORT`, so neither the developer's git config
...
fn atc(repo: &Path, args: &[&str]) -> Output {
    command(repo).args(args).output().expect("spawn atc")
}

#[test]
fn a_port_that_does_not_parse_is_the_usage_envelope() {
    let repo = Repo::new();
    let out = atc(repo.path(), &["serve", "--json", "--port", "banana"]);
...
fn the_flag_beats_the_environment_beats_the_config() {
    ...
    let server = Server::start(
        repo.path(),
        &["--port", &flag.to_string()],
        &[("ATC_PORT", &env.to_string())],
    );
    assert_eq!(server.port(), flag);
    drop(server);

    let server = Server::start(repo.path(), &[], &[("ATC_PORT", &env.to_string())]);
...
fn a_second_server_on_one_port_is_refused_by_the_socket() {
    ...
    let out = atc(repo.path(), &["serve", "--json", "--port", &port]);
    let envelope = refusal(&out, 1, "serve/address-in-use");
    let message = envelope["error"]["message"].as_str().expect("a message");
    assert!(message.contains(&port), "{message}");
    assert_eq!(
        envelope["error"]["exits"][0],
        serde_json::json!("atc serve --port <n>")
    );
}
...
fn a_host_that_does_not_parse_is_the_usage_envelope() {
    let repo = Repo::new();
    let out = atc(repo.path(), &["serve", "--json", "--host", "banana"]);
    ...
    assert_eq!(
        envelope["error"]["exits"][0],
        serde_json::json!("atc serve --host <addr>")
    );
}
...
fn localhost_is_refused_and_the_refusal_names_the_spelling_that_works() {
    ...
    let out = atc(repo.path(), &["serve", "--json", "--host", "localhost"]);
...
fn the_environment_and_the_config_refuse_a_host_the_same_way() {
    ...
    let out = command(repo.path())
        .args(["serve", "--json"])
        .env("ATC_HOST", "localhost")
        .output()
        .expect("spawn atc");
    let message = refusal(&out, 2, "usage/bad-host")["error"]["message"].to_string();
    assert!(message.contains("ATC_HOST"), "{message}");

    // The registry refuses an unusable address on the way in, so the only
    // way to have one stored is to have stored it with git.
    let out = atc(repo.path(), &["config", "--json", "serveHost", "localhost"]);
    refusal(&out, 2, "usage/bad-value");
