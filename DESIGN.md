...
- **Filters and saved views.** The filter bar and the CLI's flags want the same thing underneath — composable predicates over status, priority, label, assignee, and procedure, a saved default per person, and named views that encode into a URL. What is open is where the predicates live: parsed once in core and shared, or written twice against the same envelope. `--closed` landed ahead of all that on purpose and stays CLI-only: the closed window is one scalar the fold already took as an argument, not a predicate over rows, so handing it to the command line cost nothing and settles nothing about where the predicates live.
