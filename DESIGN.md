...
| `atc trigger` | what a wired client runs on every event it offers, dispatched on the payload's event name in three classes: a context boundary prints the notice — the flights In Progress under your callsign, pointing at `atc brief`, else the Ready count, pointing at `atc` when there is something to pull — and renews the session's lease; activity renews the lease and prints nothing; the session's end releases it. The hook runs it under a short box, and it exits 0 saying nothing whatever went wrong | hook |
