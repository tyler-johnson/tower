//! `atc doctor` against real repositories: the healthy ok row, the
...
/// The spawn, with the `ATC_FF` seam: a named program reaches the
...
fn a_fresh_repository_is_healthy() {
    ...
    let out = atc(repo.path(), &["doctor", "--json"]);
    assert_eq!(out.status.code(), Some(0));
...
"#,
    ...
    let out = atc_via(repo.path(), &["doctor", "--json"], Some(fake.path()));
