//! `atc version` against the real binary. Every run sits in a plain
//! tempdir outside any repository with `ATC_FF=/nonexistent` — the verb
...
/// cannot drift from `atc version`, and `-V` — what almost every
...
fn the_version_is_asked_three_ways_and_answered_once() {
    ...
    let long = atc(dir.path(), &["--version"]);
    let short = atc(dir.path(), &["-v"]);
    let verb = atc(dir.path(), &["version"]);
    ...
    assert!(
        ...
        "atc version does not lead with the flag's line: {:?} vs {line:?}",
        stdout(&verb)
    );

    // `-V` is gone as a spelling and present as an answer.
    let shouted = atc(dir.path(), &["-V"]);
    assert!(!shouted.status.success(), "-V no longer prints a version");
    let err = String::from_utf8_lossy(&shouted.stderr).to_string();
    assert!(err.contains("atc -v"), "names the spelling: {err}");
    assert!(err.contains("atc version"), "names the verb: {err}");
}

/// The envelope names the verb that ran. `atc -v --json` settles as
...
fn the_version_flag_does_not_ride_another_verb() {
    let dir = tempfile::TempDir::new().unwrap();
    let out = atc(dir.path(), &["-v", "board"]);
    assert_eq!(out.status.code(), Some(2), "usage error: {:?}", out.status);
    let err = String::from_utf8_lossy(&out.stderr).to_string();
    assert!(err.contains("atc -v"), "names the flag: {err}");
    assert!(err.contains("atc version"), "names the verb: {err}");
...
fn version_json_splits_the_line_into_fields() {
    ...
    if let Some(commit) = commit.as_str() {
        ...
        let line = stdout(&atc(dir.path(), &["-v"]));
