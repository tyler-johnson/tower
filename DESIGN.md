...
*The design of the tower that shipped, September 2026. It began as a sketch in August; this is what stands.*

**tower** is project management for people and agents, with a board stored as ordinary git refs beside the repository's history. It installs one standalone binary, `atc`. Bare `atc` shows the board; its verbs file, claim, brief, and finish work.

The name describes the job: tower assigns work and keeps the record. People and agents fly it.
...
The repository stores the model: mutations append events to ordinary Git refs, and the board folds the writer logs. Optional synchronization shares those logs and one counter. The refs remain legible to `git log` without tower.

> **Intent is stored; the board is derived.**

Status is a projection of stored facts: in backlog, started, closed, a question, and the edges. The fold reads no network or working-tree state. Commands can synchronously synchronize under a total deadline, then render available local state.

So the shape of the product is a boring model with an interesting engine. The model — flights, statuses, assignees, priorities, labels, sub-flights — is the one everyone already knows, and you could describe it to a Linear user in one breath. The engine underneath is an append-only record on refs and one derivation over it that every surface renders. Delete the engine and a small ordinary tracker remains; that property is deliberate and load-bearing.

## The interface

tower has its own release, authority, store, and cadence. People reach it from the shell or browser; agents reach it through the shell, with a notice and manual installed by the binary. Adapters extend the CLI through PATH dispatch and an optional declaration.

The contract:

```
reached   atc, in the current directory; atc hook wires agent clients and shells
envelope  {"atc": 1, "cmd": "<verb>", data | error} — wire ids, exit codes 0–4
stores    refs/tower/log/<author>/<writer>
serves    atc serve — the browser board, HTTP API, and SSE feed
spawns    ff version in doctor; optional ff watch --all in serve for repository motion
extends   atc-<name> on PATH; atc adapter <name> declares its capabilities
```

Only tower writes tower state. Adapters call tower's verbs to change the board. The CLI is the agent's machine surface; there is no MCP server.

## Passive by construction

**tower is a thing agents call. It never calls agents.**

Passive is a statement about initiative, not about process count. tower starts no agent work and owns no agent lifecycle; it does not claim that no tower process runs. Board mutations append local events, and there is no agent dispatch or iteration verb. If work should loop, the agent harness loops and calls `atc next` again — the harness is the scheduler, tower is only the queue.

`atc serve` is a standing process, a daemon by any honest reading, and it stays inside the line because it subscribes to motion and refolds the board. Upstream polling is a later adapter capability. It holds no board authority beyond the log, decides nothing, and dispatches no agent work. The CLI works independently; a person starts the browser server, and tower never starts it for them.

The reasons to stay passive compound. Initiating means owning agent lifecycle: keys, model selection, retries, context limits, per-vendor quirks — a second product, and a moving one. Staying passive makes tower vendor-neutral by construction: client-specific wiring delivers context and maintains the lease, while the queue records the caller's byline. A queue that dispatches on its own would make outward-facing decisions nobody asked for that minute.
...
Shared-board sync runs on ordinary board touches and the standing process's cadence through one bounded synchronous operation. Upstream tracker integration remains an adapter's responsibility. Anything that needs to reach you unasked belongs in a client or process the user started.

The passive update lane lets official binaries (never dev, dogfood, or test builds; never under CI) spawn a detached `atc update --check` at most once per `tower.updateCheck` (default daily). It refreshes a small cache file under the user cache directory and exits; foreground commands read the cache and announce each newer release at most once, with the command that owns updating this install. `tower.updateCheck false` disables checks and notices. `atc update` resolves the executable's symlinks and dispatches by channel: Source names `cargo install --git https://github.com/tyler-johnson/tower atc-cli`, Homebrew names `brew upgrade atc`, Script names the platform installer, and Unmanaged names the releases page. Only a binary at the install script's default destination belongs to Script, and only that channel can run, after `-y` or a typed yes. Without a terminal the printed command is the answer; `-y` on other channels refuses. The installer verifies the release's sha256 over HTTPS, replaces the busy executable, and runs `atc hook -u` through the newly placed binary to refresh existing wiring.

tower enforces nothing. It stores the word and shows it: a flight set In Progress is In Progress because someone said so, and the record says who and when. Its client hooks deliver context and maintain session leases; they never veto an action or reach into the tree to check a status.
...
**An orphan ref per writer.** `refs/tower/log/<author>/<writer>` — a commit chain with its own tree, no relation to code history, never touching the working tree, CAS-appended, reachability as the gc pin. Sync is one explicit refspec. The writer component is the machine's, minted once into local config: one ref per author alone breaks the moment two machines append under one email — both chains diverge and a push is rejected with no merge available, because a commit chain has no union — while a ref per writer makes every push a fast-forward, and the fold unions `refs/tower/log/**` either way.

**Every event carries four stamps.** The writer, which chain; the author, whose email; the session, which run — the first session variable set, the launcher's `ATC_SESSION` before the client's own, the login name at a terminal; and the callsign, which pilot. The first three are provenance, and none of them says who is flying a flight: an email is a person's every machine, a session is one run. The callsign is a chosen readable name for a pilot, person or agent, shared across machines on purpose — the same role on two machines is one pilot — and it comes from `ATC_CALLSIGN` when the launcher sets one, else the word the session gave `atc callsign`, held on the session's lease, else the client the process runs under, detected from the mark every agent client leaves on its shell — `CLAUDECODE` is claude, `QWEN_CODE` is qwen, `CURSOR_AGENT` is cursor, `CODEX_SANDBOX` is codex, `OPENCODE` is opencode, and so is `OPENCODE_SESSION_ID`, the variable tower's own plugin sets — else the login name at a terminal, else none. Under Claude Code `me` stores `claude` with nothing configured, and the variable is the override for a launcher that names its own pilot, the way a headless `claude -p` running a review exports `qwen-review` over the mark beneath it.
...
**Sync has three tiers.** Machine-local caches, writer identity, provisional guesses, locks, and cadence state stay local. Roaming and shared boards exchange per-writer logs through `tower.remote`. Unset, the board stays local. Remote touches fetch into staging refs, validate chains, fast-forward canonical refs, claim outstanding local filings, and push this writer's chain. Divergent chains are refused and preserved.

Tower supports a shared board through Git. It adds no notification, identity, or permission service. Compared with beads' synced-database approach, tower exchanges independently appendable writer chains and folds their union. There are no replicated mutable database cells requiring cell-level merge. Each log remains single-writer even when everyone reads it.

Events remain attributed facts, ordered by wall clock with the writer tiebreak. The sole negotiated exception is `refs/tower/seq`, a counter commit chain. A remote claim pushes a successor with an explicit expected-tip lease; Git atomically chooses a winner. A range uses one reservation and push. Assignments are recorded as `numbered` events on the reserving writer's log. Crashes can leave gaps, which are never reused. Local allocation holds a common-Git-directory lock shared by writers and linked worktrees.

The log is durable state. Ordinary sync defaults to a 30-second cadence and a three-second total deadline; failed attempts also observe cadence. Filing and decomposition use ten seconds for allocation independently of cadence. Transport is owned and cleaned up before return. Enrollment is keyed by the counter root's object ID. Joining another established domain requires confirmation before local chains publish; reconnecting to the same domain does not renumber again. Pending reservations and enrollment progress survive interruption in the common Git directory. See the [transaction protocol](docs/shared-board-protocol.md).
...
At work the team already has a tracker. tower does not replace it and is never authoritative over it. Linear and GitHub are first-class foreign writers, observed and absorbed, never owned. No upstream adapter exists yet, and solo mode is the only mode that runs today, but the adapter interface is built, the ownership table is the design's, and the local layer already keeps to its row.
...
**Local steps stay local until promoted.** A team ticket decomposes into steps that are real, tracked, briefed, and assignable — and invisible upstream. A step that turns out to need a teammate or a PR of its own gets promoted, which mints a real upstream ticket, links it, and keeps the local history. The team's board stays as coarse as the team wants, the local board is as fine as the work actually is, and neither has to negotiate with the other. Promotion is not built; it waits on the adapter it would mint through.

Adapters are separate executables, `atc-<name>` on PATH, reached through `atc <name>`. Dispatch works without a declaration; `atc adapter <name>` records a checked manifest and adds help, explanations, notice lines, skills, update recipes, and diagnostics. The [extension contract](docs/README.md#extensions) defines what an adapter supplies. Solo mode is the case where no upstream adapter is installed.

## Surfaces

One model, every renderer: every surface is a thin shell over the same contract the CLI renders, never a second implementation.
...
An agent reaches every verb from the shell, with the same envelope and exit codes available to a script. Declared adapters answer manifest and skill handshakes; tower serves no MCP tools.

Bare `atc` answers from the local log after a cadence-gated synchronous sync attempt. The complete ordinary attempt is bounded by `tower.syncTimeout`, three seconds by default. The fold performs no network I/O. Failed connectivity leaves available local state usable; provisional numbers are visibly marked with a tilde.
...
A flight has four naming forms. Its permanent wire ID is its `filed` event's `<writer>.<seq>`. Every event kind, including `numbered`, consumes a sequence ID, so wire IDs are sparse. JSON carries the wire ID unchanged.

`writer#n` is the permanent ordinal alias: rank among that writer's filings, derived by sequence and never stored. `#n` is the latest confirmed global claim, stored in a `numbered` event and printed identically by every synchronized writer. Claims follow arrival order; offline work can receive a higher number than newer online work.

Until claimed, a flight is `~n`: the locally observed counter plus one plus earlier provisional filings on its writer's chain. `writer~n` disambiguates overlapping guesses. Guesses are derived from explicit local context and never stored as aliases. Tilde inputs name currently provisional flights only; bare numeric input means a global claim. A leading `#` is accepted for paste tolerance. Quote shell arguments containing `#` or leading `~` when needed.

Board rows, briefs, and picks carry `id`, `writer`, and `display`, with no bare `number` field. Brief links and parents carry `writer` and `display` beside their `flight` wire ID. `board::display` is the sole claimed-or-provisional display rule against the complete fold. Clients print it as supplied. Legacy single-writer logs migrate while preserving their numbers; multi-writer logs receive unique claims in deterministic fold order.

A flight named in prose is stored as `#<wire id>`. Every prose-taking verb resolves global numbers, ordinal aliases, and provisional forms against its pre-sync snapshot, then carries wire IDs through synchronization. Advancing a guess cannot retarget the command. Unresolved text stays as typed. Stored references survive renumbering; terminal and web projection use the current display without rewriting the log.

### Next

`atc next [<lane>...]` is a queue's Ready check and the claim in one command. The lanes are the arguments — `me`, `agent`, `none`, or a callsign, walked in the order given, and nothing appended: the walk is exactly the lanes named, so `next agent` walks the pool alone and Ready work in the unassigned lane is `elsewhere` to it. `me` is the literal `me` lane and the caller's callsign, or the literal alone for a caller with none; `agent` is the shared pool alone, `none` the unassigned lane, a callsign that pilot's queue. No lane named is the caller's default, a function of who is calling: under a client mark the walk is `me`, the client's lane, `agent`, `none`: its own queue, the lane every session of its client draws from, the shared pool, then the unassigned lane, with the client's lane left out when the resolved callsign already is the client word, since `me` admits it then; with no mark — a person at a shell, a hook with nothing set, `ATC_CALLSIGN` in a bare shell — the walk is `me`, `none`. The client lane rides on the mark whatever the callsign's source, so a headless `claude -p` under `ATC_CALLSIGN=qwen-review` still draws from `claude` after its own queue, and a launcher that wants narrower names its lanes. Each lane walks by priority, then filed order — the board's own order, so an urgent flight filed late leads its lane and never an earlier one; a lane named twice walks once, and a flight in two named lanes is picked once, in the first — and `-n <k>` admits up to `k` of the walk. Each picked flight is set In Progress in one append, and the callsign on the event is the pilot: the pick is the claim, and nothing else. The pick lands in the caller's own queue by default — `--assignee` defaults to `me`, which stores the callsign — so a worker claiming from the pool takes the flight into its own queue in one gesture, and a flight it later releases with `status ready` waits in that queue rather than falling back into the pool; `--assignee <lane>` says where it lands instead, with `file`'s words, and `--assignee agent` is how a pick stays in the pool. The re-lane rides the same append and is written only when the lane changes: a flight pulled from its own queue is one moment on the record, not two. `next` hands out no branch and no tree — where the work happens is the harness's to arrange, one tree per flight when it fans out — and a flight with a live dependency is Waiting, in no walk. `--peek` is the same computation with nothing written, and the envelope's `pulled` says which happened. A bad lane word on either side is `usage/bad-assignee`, the refusal `assign` raises.

Three outcomes, and the envelope names each on `outcome`. `work`, exit 0, is a pick: the `picked` rows carry `id`, `writer`, `display`, `subject`, and the `skill` the flight names when it names one. `drained`, exit 1, is a board with nothing Ready anywhere. `elsewhere`, exit 1, is Ready work in a lane the walk never entered — the count rides beside it as `elsewhere`. Both empties are full data envelopes: a shell loop stops on the code, and a JSON reader branches on the word.

### Brief

`atc brief` is the read half of the handoff: `next` hands an agent a flight id and a subject, and the brief is what it reads next — everything the log knows about one flight, in one read over the fold. It is the record and nothing but: subject, body, every field, the newest handoff pinned above the comments, the comments with any question and answer among them, links carrying each linked flight's subject and status, each parent as a link row, `referenced by` — the flights whose prose names this one, in the link rows' shape — the history with the byline and the words each verb took, and the standing. The backlink is discovered-from with no edge kind: a flight filed while flying another says so in its body, and the other's brief shows it. The handoff is a comment flagged as the state of play — `atc comment <flight> --handoff -m "…"` — append-only and attributed like any other, so there is no mutable notes field to overwrite: the brief pins the newest and every prior one stays in the stream, flagged. It never holds the flight; the question stays its own kind. Each parent's body rides the wire with its row, one level up and no further: a flight minted by `decompose` has a thin body and its real context is the parent's, so a script reading the envelope has the whole without a second read. The CLI prints the parents as link rows and the body under each only under `--expand` — `blocks` is every dependent, and the model keeps no distinction between an edge `decompose` minted and one `link` wrote, so a sequencing dependency reads as a parent too — and the web record lists the row alone. No repository facts and no probes, so the brief is instant and reads the same from any tree. A closed flight briefs like any other, because the log keeps the record and reading it is never a lifecycle move. `show` is accepted as a second spelling of `brief`; the envelope says `brief` either way.

The skill named on the flight is what the agent flies it with; the brief is what the agent flies it *from*, and it is why the asker of a held question does not need to be its resumer. The body and the comments are authored prose, and the reader decides how it reads: the web board renders them as markdown, the terminal prints the source text, with references projected.
...
Every verb must earn its existence, and the one that fails first is `run`. tower never runs agent work: the harness schedules and executes it.
...
| `atc next [<lane>...] [--assignee <lane>] [-n <k>] [--peek]` | claim the next Ready flight from the lanes named in order, and only those — unsaid, your own queue, your client's lane, the pool, then the unassigned lane; a person's own queue then the unassigned lane — or `k` of them by priority, then filed order; the pick sets In Progress under your callsign and moves the flight into your queue unless `--assignee` says where; `--peek` is the same computation with nothing written | an agent |
| `atc brief <flight>` (alias `show`) | everything the record knows about one flight, for whoever picks it up | either |
| `atc file [<procedure>] <subject>` | put work on the board — bare, or under a procedure; every field a procedure sets is a flag here (`-m`, `-p`, `--label`, `--skill`, `--assignee`, `--status`); `--json` also returns board rows in `flights`, the filing first, then its parts | either |
...
| `atc comment <flight> [--handoff] -m <note>` | a note on the record, local; saying it to the team is a separate, deliberate gesture. `--handoff` flags it as the state of play, and the brief pins the newest | either |
...
| `atc decompose <flight> [<procedure> \| <part>…]` | make a flight a parent — a procedure's flights, or parts by hand; `--json` also returns board rows in `flights`, the updated parent first, then the new parts | either |
...
| `atc config` | settings in a typed registry | you |
...
| `atc update` | name this install's update command; run the installer only after `-y` or a typed yes; a passive lane checks ~daily and announces each release once | you |
| `atc doctor` | the optional repository interface, log, client wiring, procedures, skills, declared adapters, and update cache; missing `ff` is informational; observes and complains, never enforces, and exits 1 on findings | you |
| `atc adapter [<name>]` | list declarations, or ask an adapter for its manifest and record it; `-d <name>` forgets a declaration | you |
| `atc hook [<client>...]` | wire clients and shells; `-l` reports, `--all` takes those detected, and `-u` refreshes declarations and existing wiring | you |
| `atc unhook [<client>...]` | remove exactly tower's wiring for the named clients and shells, or `--all` for those detected | you |
| `atc serve` | run the standing process: a server the browser board and its API mount into, in the foreground until Ctrl-C; `--host`, then `ATC_HOST`, then `tower.serveHost`, then 127.0.0.1, and `--port`, then `ATC_PORT`, then `tower.servePort`, then 7420. The default is the loopback; a wider bind works and says once that the board has no authentication in front of it. Mounted: the board itself — the web app embedded in the binary at build time, every path outside `/api` answering a build file or the app shell, the client router taking it from there; the read API — GET /api/board, /api/brief/<flight>, /api/procedures, /api/views — each a fresh fold answering the same envelope the verb emits under `--json`, `{"atc": 1, "cmd": "<verb>", …}`, and `/api/board?<query>` taking the query string the views store, answering the groups plus the hidden and filtered counts, and refusing one it cannot parse as a 400; the verb API — POST /api/file, /api/status, /api/assign, /api/hold, /api/answer, /api/done, /api/cancel, /api/comment, /api/decompose, /api/edit, /api/link, /api/unlink, /api/views/save, /api/views/edit, /api/views/delete — each taking the verb's arguments as a JSON body, appending to the log, and answering the verb's own envelope; and the change feed — GET /api/feed, one SSE stream per query, taking the same query on its URL and pushing each subscriber's own fold whenever the repository moves, whoever moved it, every frame being `/api/board?<query>`'s body minus the newline: the server stamps motion when its filesystem watcher sees the log refs or the optional `ff watch --all` child sees the repository. The filesystem watcher works without that child; each subscriber folds against its own query, and a POST publishes nothing directly, so every writer's board arrives the same way | you |
| `atc trigger` | what a wired client runs on every event it offers, dispatched on the payload's event name in three classes: a context boundary prints the notice — the flights In Progress under your callsign, pointing at `atc brief`, else the Ready count, pointing at `atc` when there is something to pull — and renews the session's lease; activity renews the lease and prints nothing, and once per `leaseSweep` sweeps the leases past `leaseExpiry`; the session's end releases it. The `shell` source is a terminal's rc lines: no payload, a heartbeat before every prompt, `--end` the release at exit, and never a notice. The hook runs it under a short box, and it exits 0 saying nothing whatever went wrong | hook |

These verbs answer a caller's request: reads fold local state, mutations append events, and installation verbs change the caller's wiring or declarations. `serve` is a standing process and still decides nothing. Nothing in the column on the right is an agent dispatch target.

Promotion — minting the upstream ticket for a local step, linking it, keeping the local history — is the publish boundary described under *Upstream is a foreign writer*. It waits on an upstream adapter and is not spelled as a command line until it parses.
...
**A procedure should end with you.** A shape of work normally ends with a human review, so the loader warns when every terminal flight is agent-assigned. Shared-board synchronization is authorized by configuring `tower.remote`; publishing work to an upstream tracker remains a separate human gesture.
...
A skill holds instructions a harness executes, never a process tower spawns. The hook installs tower's manual; a flight's `skill` field names workflow instructions on the user's shelf. The harness runs the judgment, and tower never grows a process supervisor.
...
tower ships one skill and one mechanism, and no workflow. The skill is the manual, `tower` — the model, the envelope, the exit codes, the landmines, and where every verb's own `--help` is the last word. The mechanism is the `skill` field on a flight, the shelf that field names into — user, then repository, the same name replacing wholesale, like procedures — and `atc skills <name>`, which prints one raw, byte for byte. Nothing on the shelf is the binary's, and the manual never appears there. The documentation's examples cover the recurring three: **plan** (decompose a goal into a tree of flights — solo mode's entry point), **work** (claim, fly, hold or hand off, finish, repeat — the loop that pairs with `next`), and **review** (first-pass someone else's branch: commit the mechanical fixes, write the pass as a comment, hold the judgment for a person's verdict). They are one workflow among many, and the shape of the loop is the owner's to fork.

Each agent-assigned flight names the skill it is flown with, `next` hands out that name on the picked row, and the agent prints it with `atc skills <name>` and follows it for that flight. The flight carries the name, keeping structure in data and judgment in prose.
...
Fan-out is `atc next -n 3` handing out three flights. One tree per flight is the harness's to arrange; tower says nothing about where the work happens, and the pick is the same claim whether one worker takes it or three.
...
2. **tower is called; it never calls agents.** No agent dispatch or loop. A standing process may refold and subscribe; it decides nothing. The harness schedules; tower queues.
...
5. **Store the word, show the word, enforce nothing.** A status is what someone said, attributed; tower does not veto actions or check a status against a tree.
6. **Conflict-free by construction.** Union-merged event logs, not a synced database.
7. **Local work stays local until promoted.** Sub-flights stay on the local board; promotion is the publish boundary.
8. **Deferred requires loud.** A held flight is announced, pinned, and blocks its exits.
9. **One model, every surface.** CLI, the web, and anything later consume one contract.
10. **Exactly one negotiated fact.** Attributed events stay on their writers' chains. Tower negotiates only the numbering counter, using Git's atomic ref update. It adds no second consensus mechanism.
...
The hook is in fufu's shape: `atc hook` detects clients and shells, reports, then asks; names wire exactly those, `--all` wires everything detected without asking, and `-l` only reports. The binary owns the notice, manual, and client-specific wiring. Each plugin carries the `tower` manual and skills supplied by declared adapters; workflow examples stay in `docs/skills` for their owners to copy and fork.

| client | what hook installs | what unhook removes |
|---|---|---|
| Claude Code (`claude`) | its own plugin at `~/.claude/skills/tower`; `--settings` instead merges hooks into `~/.claude/settings.json` and installs no skills | the plugin and tower's settings entries, including legacy wiring |
| Codex (`codex`) | a plugin at `~/.agents/plugins/tower`, one entry in the personal marketplace beside it, and client registration; hooks need review through `/hooks` | the plugin and tower's marketplace entry; the report names the command to clear Codex's cache |
| Qwen Code (`qwen`) | hook entries merged into `~/.qwen/settings.json`; the notice alone, because Qwen reads no skills directory | tower's hook entries |
| OpenCode (`opencode`) | `~/.config/opencode/plugins/tower.js` and the manual at `~/.config/opencode/skills/tower`, with adapter skills beside it | that module and the skills tower installed |
| Copilot CLI (`copilot`) | an Agent Plugins 1.0 plugin at `~/.agents/plugins/copilot/tower`, a dedicated marketplace beside it, and `tower@tower-atc` registration in `~/.copilot/settings.json` | the plugin, dedicated marketplace, and tower's registration |
| Cursor CLI (`cursor`) | a native plugin at `~/.cursor/plugins/local/tower`, discovered on a new session | that plugin and tower's legacy hook entries and skills |

**The trigger has three event classes.** A context boundary prints the notice and renews the session's lease. Activity renews the lease silently and, once per `leaseSweep`, sweeps leases past `leaseExpiry`. The session's end releases its lease. Each client wires the events its hook interface supports; a turn ending is activity, not a session ending. Unknown events and failures exit 0 without output. `atc briefing` remains a compatibility spelling for the notice.

OpenCode's module puts the notice in the system prompt on every model call, so it is standing and survives compaction. It also sets `OPENCODE_SESSION_ID` on shell commands and renews the lease on tool activity. The other clients receive the notice at the context boundaries their hooks expose. The notice points an active pilot at `atc brief` and otherwise reports the board's readiness; it never claims work.

The shell slugs are `bash`, `zsh`, `fish`, and `powershell`. Marked rc lines mint `ATC_SHELL_SESSION` once per interactive shell, set its pid, run `atc trigger shell` before each prompt, and release the lease at exit. Shell triggers print no notice. A client launched from that shell resolves its own session ahead of the terminal's.

**Refresh and removal follow ownership.** `atc hook -u` first re-asks declared adapters for their manifests, even when no client is wired, then repairs existing installs without adding unwired clients. It refreshes the binary path, notice wiring, manual, and adapter skills; a failed skill reply preserves the older installed copy, and retired skills are removed. `atc unhook` removes tower-owned plugin paths whole and removes only tower's entries or marked lines from shared files. Foreign entries and manually authored trigger lines remain. Hook verifies replacement plugins before removing legacy wiring.

**fufu is an optional interface to repository history.** The board, CLI, hooks, and HTTP API work without it. Today `serve` optionally spawns `ff watch --all` to observe repository motion beside its own filesystem watcher over the log refs; that watcher keeps the board live when `ff` is absent, and no render waits on the child. `doctor` probes `ff version` and reports absence as information, not a finding. The later changes layer can use repository facts through this optional interface without making them prerequisites for the board.

## What it waits on

Load-bearing and absent:

- **Forge reads.** A review shape stands almost entirely on state the repository cannot see, so the adapter that supplies it is a dependency of the documented examples rather than a nicety. The adapter interface exists; the forge adapter and promotion remain to be built.

Everything the record needs exists today: the store, the fold, the hook, the pick, the brief, the hold. What is missing is the layer above the record, and *The later layer* is its order.
...
- **What a flight means after a rewrite** folds its snapshots into a commit — a session-boundary question made urgent the day an event carries a snapshot id.
