...
//! bare `atc config` prints, and a value is validated through the
...
pub enum Error {
    ...
    #[error("unknown setting \"{input}\" — `atc config` lists them all")]
...
/// The port `atc serve` binds when nothing else says otherwise —
/// the last of the four lanes, behind `--port`, `ATC_PORT`, and
...
/// The address `atc serve` binds when nothing else says otherwise,
/// behind `--host`, `ATC_HOST`, and `tower.serveHost`. A `&str` rather
...
/// The status a bare `atc file` gives a flight when nothing else
...
pub fn registry() -> &'static [Setting] {
    &[
        Setting {
            ...
            desc: &[
                "The status `atc file` gives a flight that says nothing else:",
        ...
        Setting {
            ...
            desc: &[
                "The address `atc serve` binds. --host beats ATC_HOST beats",
        ...
        Setting {
            ...
            desc: &[
                "The port `atc serve` binds. --port beats ATC_PORT beats",
