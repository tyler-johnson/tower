...
Four events move a flight through its states, and each one is a person or an agent saying something, not tower deriving anything. `claimed` marks a flight in the air at the moment of assignment — before any capture exists, the claim itself is the motion, so the board answers "who has this" without waiting for a keystroke in the bay. `held` attaches a question and moves the flight to **waiting on you** with its bay intact: branch and tip stay on the row, because a warm bay is the point of holding rather than abandoning. `answered` releases the hold — the question clears, the answer goes on the record as the flight's freshest motion, and the flight returns to wherever the partition puts it. `done` takes the flight off the board: out of every section, out of the flight count, and out of the JSON — a `--json` consumer has no "done" list, deliberately, because the board shows what is live and the log keeps everything else. The record survives in full; `comment` and `link` still land on a done flight, and its id still resolves.
...
| `ff tower serve` | run the standing process: a server the browser board and its API mount into, in the foreground until Ctrl-C; `--host`, then `TOWER_HOST`, then `tower.serveHost`, then 127.0.0.1, and `--port`, then `TOWER_PORT`, then `tower.servePort`, then 7420. The default is the loopback; a wider bind works and says once that the board has no authentication in front of it. Mounted so far: the read API — GET /api/board, /api/brief/<flight>, /api/bays, /api/procedures — each a fresh fold answering the same envelope the verb emits under `--json`; and the verb API — POST /api/file, /api/claim, /api/take, /api/requeue, /api/hold, /api/answer, /api/done, /api/comment, /api/triage — each taking the verb's arguments as a JSON body, appending to the log, and answering the verb's own envelope; `/api/triage` takes `flight`, `procedure` and an optional `message`, and routes the flight to the named procedure; and the change feed — GET /api/feed, one SSE stream pushing the full board envelope whenever the repository moves, whoever moved it: the server refolds when its watcher sees the log refs or `ff watch --all` sees the repository, and a POST publishes nothing directly, so every writer's board arrives the same way | you |
