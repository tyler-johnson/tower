...
| `atc next [<lane>...] [--assignee <lane>] [-n <k>] [--peek]` | claim the next Ready flight from the lanes named in order — your own queue when unsaid, then the unassigned overflow — or `k` of them by priority, then filed order; the pick sets In Progress under your callsign and moves the flight into your queue unless `--assignee` says where; `--peek` is the same computation with nothing written | an agent |
...
Fan-out is `atc next me agent -n 3` handing out three flights. One tree per flight is the harness's to arrange; tower says nothing about where the work happens, and the pick is the same claim whether one worker takes it or three.
