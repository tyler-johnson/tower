...
fn main() {
    ...
    if cli.version_shouted {
        report(
            ...
            &CliError::coded(
                ...
                vec!["atc -v".into(), "atc version".into()],
            ),
        );
    }
    ...
    // `--closed` sits on `Cli` so bare `atc --closed 7d` parses,
    ...
    if cli.board.closed.is_some() && !matches!(cli.command, None | Some(Command::Board { .. })) {
        report(
            ...
            &CliError::coded(
                ...
                vec!["atc --closed 7d".into(), "atc board --closed 7d".into()],
            ),
        );
    }
    // The lanes are decided from the command line, before anything runs:
    // bare `atc` is the board and rides the board's, and bare
    // `atc -v` is the version verb spelled as a flag, so it takes
...
/// caller always has an envelope to parse; a human gets `atc: {err}`
