//! `atc config` — read and write tower settings stored as plain
...
use atc_core::config::{self, Config, Setting, registry};
...
fn list(json: bool, config: &Config) -> Result<(), CliError> {
    ...
    println!(
        "{}",
        render::paint_dim(
            "Set with:     atc config <key> <value>   (--global: every repo)",
            colored
        )
    );
    println!(
        "{}",
        render::paint_dim("Remove with:  atc config --unset <key>", colored)
    );
    println!(
        "{}",
