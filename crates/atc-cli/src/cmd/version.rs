//! `atc version` — which tower this is, and whether it is the
//! current one.
//!
...
pub fn run(json: bool) -> Result<(), CliError> {
    ...
    if json {
        let payload = serde_json::json!({
            ...
            "commit": some(env!("ATC_BUILD_SHA")),
            "date": some(env!("ATC_BUILD_DATE")),
            "update": {
                "status": match &status {
    ...
    if let CheckStatus::Available(tag) = &status {
        println!(
            "{}",
            render::paint_dim(&format!("{tag} available — atc update"), render::colored())
        );
    }
    Ok(())
}
