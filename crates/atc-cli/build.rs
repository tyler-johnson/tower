//! Emit the build provenance so `atc -v` and `atc version` can
//! name the commit the binary came from.
//!
//! Three variables, one fact. `ATC_BUILD_INFO` is the display half —
//! either ` (<sha> <date>)` or the empty string — and `ATC_BUILD_SHA` /
//! `ATC_BUILD_DATE` are the same two values on their own, because
//! `atc version --json` reports them as fields and parsing them back
...
fn main() {
    ...
    println!("cargo::rustc-env=ATC_BUILD_INFO={info}");
    println!("cargo::rustc-env=ATC_BUILD_SHA={sha}");
    println!("cargo::rustc-env=ATC_BUILD_DATE={date}");
}
