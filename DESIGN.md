...
- **The trigger.** A wired client runs `atc trigger` on every event it offers: at a context boundary the notice, one line, a short box, dropped whole past either; on activity a heartbeat on the session's lease, and once per `leaseSweep` a sweep of the leases past `leaseExpiry`; at the end its release.
