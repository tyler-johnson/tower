...
<div class="join">
  ...
  <details class="dropdown dropdown-end" bind:open {@attach dismiss()}>
    ...
    {#if open}
      <div
        class="dropdown-content rounded-box border-base-300 bg-base-100 z-10 flex w-64 flex-col gap-2 border p-2 text-sm shadow-sm"
      >
        ...
        <label class="flex flex-col gap-1">
          <span class="flex items-center gap-2">
            <input
              ...
              checked={view.shared}
              disabled={views.busy}
              onchange={(event) => views.edit(view.id, { shared: event.currentTarget.checked })}
            />
        ...
        <button
          class="btn btn-sm btn-ghost text-error self-start"
          disabled={views.busy}
          onclick={remove}
        >
