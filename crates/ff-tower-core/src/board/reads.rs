//! The module's I/O: three fufu spawns, the collide probes, and two
//! structs of answers.
//!
//! Three spawns constant in flight count, plus one `collide` per
//! distinct pair of in-flight branches — zero probes in the solo norm,
//! where fewer than two distinct branches are flying. Every fufu read
//! takes a capture first, so spawn count is log noise as well as latency
//! — a read-lane call in an unchanged tree is a `NoOp` capture that
//! appends nothing. `op log` over `session(glob:*)` answers for every
//! tagged operation on the invoking worktree's chain at once, `branch
