...
pub static ENTRIES: &[Entry] = &[
    Entry {
        ...
        exits: &["atc adapter", "atc doctor"],
    },
    Entry {
        id: "adapter/not-declared",
        summary: "that adapter has no declaration on this machine",
        detail: "Removing a declaration requires its recorded name. The binary may still run from PATH without a declaration; atc adapter lists the names the registry holds.",
        exits: &["atc adapter"],
    },
    Entry {
        id: "adapter/handshake-failed",
    ...
    Entry {
        id: "adapter/unsupported-contract",
    ...
    Entry {
        id: "adapter/registry-unreadable",
        summary: "the declaration registry cannot be read",
        detail: "The per-machine adapters.json is malformed or cannot be opened. Readers describe no adapters and doctor reports the problem. Writers refuse to overwrite it so the existing declarations can be repaired.",
        exits: &["atc doctor"],
    },
    Entry {
        id: "adapter/registry-unwritable",
        summary: "the declaration registry cannot be written",
        detail: "The environment names no config directory, or creating or replacing adapters.json failed. The refusal names the path and IO error when one exists. Correct the directory or permissions and repeat the declaration.",
        exits: &["atc doctor"],
    },
    Entry {
        id: "adapter/delegate-failed",
        ...
        exits: &["atc doctor", "atc adapter"],
    },
    Entry {
        id: "adapter/skill-failed",
    ...
    Entry {
        ...
        detail: "`atc hook` and `atc unhook` take flat, permanent names — claude, codex, \
                 cursor, qwen, opencode, copilot, and the shells — because they end up written inside config files tower \
                 does not own and cannot rename afterward. These two verbs are for people, so \
                 an unknown name is a real error here; gemini was a name once, and \
                 its adapter went, so the hook refuses it while `atc trigger <source>` \
    ...
    Entry {
        ...
        detail: "An address here is an IP literal — 127.0.0.1, 0.0.0.0, ::1, or a tailnet \
                 ...
                 validatable offline through the same parser its reader runs. `atc serve` \
                 takes the address from the first of four lanes that speaks — --host, then \
                 ATC_HOST, then tower.serveHost, then the compiled 127.0.0.1 — and the \
        ...
        exits: &["atc serve --host <addr>"],
    },
    Entry {
        id: "usage/bad-port",
        summary: "that is not a port",
        detail: "A port is a number from 0 to 65535, and `atc serve` takes one from the \
                 first of four lanes that speaks: --port, then ATC_PORT, then \
    ...
    Entry {
        ...
        exits: &["atc --closed 10", "atc --closed 7d", "atc --closed none"],
    },
    Entry {
    ...
    Entry {
        ...
        exits: &["atc link <flight> <dependency>"],
    },
    Entry {
        ...
        exits: &["atc hold <flight> -m <question>"],
    },
    Entry {
    ...
    Entry {
        ...
        detail: "Settings live in a typed registry, fufu's model: only declared keys exist, \
                 with or without the `tower.` prefix, case-insensitive. Bare `atc config` \
    ...
    Entry {
        ...
        exits: &["atc answer <flight> -m <answer>"],
    },
    Entry {
        id: "answer/not-held",
    ...
    Entry {
        ...
        detail: "The name is in neither layer — `~/.config/tower/skills/` or the \
                 ...
                 normal state of a fresh box: bare `atc skills` names both directories.",
        exits: &["atc skills"],
    },
    Entry {
        id: "skill/invalid",
    ...
    Entry {
        ...
        detail: "The socket would not bind: another `atc serve`, or anything else holding \
...
mod tests {
    ...
    ///
    /// Tower's exits are spelled `atc …`, and the whole line parses
    /// against tower's own clap tree, with `<…>` placeholders standing
    /// in as values and bare `atc` being the board. `ff …` is fufu's
    /// surface and fufu's guards hold it; `git`, `cargo`, and `brew` are
    ...
    fn check_exit(exit: &str, whose: &str) {
        ...
        if ["ff", "git", "cargo", "brew"].contains(&first.as_str()) {
            return;
        }
        assert_eq!(
            first, "atc",
    ...
    fn raised_everywhere() -> Vec<(String, String)> {
        ...
        for table in [
            "atc-cli/src/error.rs",
            "atc-serve/src/api.rs",
            "atc-serve/src/error.rs",
            "atc-core/src/board/query.rs",
            "atc-core/src/board/resolve.rs",
            "atc-core/src/config.rs",
            "atc-core/src/verb/error.rs",
            "atc-core/src/ff/error.rs",
            "atc-core/src/log/error.rs",
            "atc-core/src/procedure/mod.rs",
            "atc-core/src/skill/mod.rs",
        ] {
    ...
    ///
    /// The filter takes literals with *exactly one* `/`, which is the
    /// standing hazard here: ids live bare in these tables and the
    /// `tower/` namespace goes on at the emit boundary, so a
    /// `tower/`-prefixed literal landing in one of the eleven scanned
    /// `fn id()` bodies would be dropped from the raised set without a
    /// word — surfacing later as `every_registry_entry_is_reachable`
    /// calling the entry an orphan, which points at the wrong cause.
