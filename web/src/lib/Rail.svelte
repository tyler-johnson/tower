...
{#snippet heading(title: string)}
  <span class="text-base-content/60 font-mono text-xs font-medium tracking-[0.2em] uppercase">
    {title}
  </span>
{/snippet}
...
-->
<aside class="rounded-box border-base-300 flex flex-col gap-4 border p-4">
          ...
          <span class="badge badge-sm gap-1">
            ...
            <button
              ...
              onclick={() =>
                set(
                  "label",
                  brief.labels.filter((label) => label !== one),
                )}
            >
              ✕
            </button>
  ...
  <!--
	...
	-->
  {#if other.length > 0}
    <div class="border-base-300 flex flex-col gap-1 border-t pt-4">
      {@render heading("other")}
      {#each other as row (row.label)}
        <p class="text-base-content/40 font-mono text-xs break-all">{row.label} · {row.value}</p>
      {/each}
    </div>
  {/if}
</aside>
