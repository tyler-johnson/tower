...
//! The prose behind each id lives in `explain.rs`'s registry — `atc
...
use atc_core::board::ResolveError;
use atc_core::{config, ff, log, procedure, skill, verb};
...
impl CliError {
    ...
    pub fn exits(&self) -> Vec<String> {
        match self {
            ...
            CliError::Skill(_) => vec!["atc skills".to_string()],
            CliError::Config(config::Error::UnknownKey { .. }) => {
                vec!["atc config".to_string()]
            }
            CliError::Config(config::Error::BadValue { .. }) => {
                vec!["atc config <key>".to_string()]
            }
            _ => Vec::new(),
        }
    }
...
impl From<atc_serve::Error> for CliError {
    fn from(err: atc_serve::Error) -> CliError {
        match err {
            atc_serve::Error::Repo(err) => CliError::Log(err),
