...
pub const ATC_NAME: &str = "tower";
pub const ATC_EMAIL: &str = "tower@local";
...
fn is_tower_commit(commit: &gix::objs::CommitRef<'_>) -> bool {
    commit.author.name == ATC_NAME
        && commit.author.email == ATC_EMAIL
        && commit.committer.name == ATC_NAME
        && commit.committer.email == ATC_EMAIL
}
