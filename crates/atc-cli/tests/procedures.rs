//! `atc procedures [<name>]` — the read-only half of the registry.
//!
...
fn an_empty_registry_says_so_and_where_a_definition_goes() {
    ...
    let repo = repo();
    let out = stdout(&atc(repo.path(), &["procedures"]));
    assert_eq!(
        out,
        format!(
...
fn the_listing_is_what_is_installed_with_flights_and_lanes() {
    ...
    assert_eq!(
        ...
        "review  repo\n\
         ...
         · work  me\n\
         \n\
         2 procedures · atc procedures <name> for one\n"
    );
}
...
fn the_detail_carries_the_flights_the_inert_rule_and_the_file() {
    let repo = stocked();
    let out = stdout(&atc(repo.path(), &["procedures", "review"]));
...
fn the_json_form_is_the_registry_as_data() {
    ...
    let one = envelope(&atc(repo.path(), &["procedures", "review", "--json"]));
...
fn a_flight_with_an_unfileable_status_is_refused_naming_the_file_and_the_flight() {
    ...
    let out = atc(repo.path(), &["procedures", "--json"]);
    ...
    let one = envelope(&atc(repo.path(), &["procedures", "parked", "--json"]));
    assert_eq!(
...
fn a_repo_definition_overrides_the_user_layers_and_says_so() {
    ...
    let detail = stdout(&atc(repo.path(), &["procedures", "review"]));
    assert!(
...
fn a_shape_that_ends_on_an_agent_loads_and_warns_on_both_renders() {
    ...
    let list = stdout(&atc(repo.path(), &["procedures"]));
    assert!(
        list.contains("! ends on agent flight pass — a procedure should end with you\n"),
        "{list}"
    );
    let detail = stdout(&atc(repo.path(), &["procedures", "script"]));
    assert!(
...
fn a_name_that_is_not_installed_is_refused_naming_the_set() {
    let repo = stocked();
    let out = atc(repo.path(), &["procedures", "ghost", "--json"]);
    assert_eq!(out.status.code(), Some(1));
    let envelope = envelope(&out);
    assert_eq!(
        envelope["error"]["id"],
    ...
    let bare = self::repo();
    let out = atc(bare.path(), &["procedures", "ghost", "--json"]);
    assert_eq!(out.status.code(), Some(1));
    assert_eq!(
