...
Sync follows the same discipline. The upstream design pulls lazily at invocation, gated by a cadence stamp. With the standing process up, that same pull would run on its cadence and append the same events — the cadence is config it reads, never authority it holds. Today the standing process subscribes to local motion; upstream sync waits on an adapter. Anything that needs to reach you unasked belongs in a client or process the user started.
...
Authored text is derivable from nothing. The log is durable state, and losing its refs can lose flights. The store is ordinary git objects in the repository, and whether they leave the machine is git's business — tower carries no backup surface, no verb, no warning, no doctor row.
...
Board rows, briefs, and picks carry `id`, `writer`, and `display`, with no bare `number` field. Brief link and parent rows carry `writer` and `display` beside their `flight` wire id. `board::display` derives the name against the complete fold, including closed flights outside the visible window; clients print it as supplied. The dense per-writer number stays inside the fold for resolution. Global number claims and provisional tilde names belong to the later shared-board work in #139.

A flight named in prose is stored as `#<wire id>`. Every verb that takes prose — `file -m`, `comment`, `hold`, `answer`, `edit -m`, a close's `-m` — scans the text at write time for `#3` and `writer#3`, resolves each by the same rule the flight argument gets, and stores `#pi-8c2e.140` in its place: a bare number two writers hold refuses with the same listing, and a match on nothing — a GitHub number, a `#1 priority`, a flight not yet filed — stays as typed, never guessed at. The stored form survives any renumbering, and every surface prints it by the current rule: the terminal substitutes `#3` or `pi-8c2e#3` into the source text, the web renders a link to the flight. The scan is write-time only; a render never rewrites the log.
