//! `atc explain` against the real binary: the error-id lookup,
//! fufu's treatment. Most runs sit in a plain tempdir outside any
//! repository with `ATC_FF=/nonexistent` — the verb is a pure registry
...
fn the_list_aligns_ids_beside_summaries() {
    let dir = tempfile::TempDir::new().unwrap();
    let text = stdout(&atc(dir.path(), &["explain", "--list"]));
...
fn json_carries_the_entry_and_the_list() {
    ...
    assert_eq!(data["exits"], serde_json::json!(["atc"]));
...
fn an_unknown_id_refuses_with_exit_2_and_names_the_list() {
    ...
    assert_eq!(
        ...
        serde_json::json!(["atc explain --list"])
    );
...
fn a_flight_shaped_argument_points_at_brief() {
    ...
    let dir = tempfile::TempDir::new().unwrap();
    let out = atc(dir.path(), &["explain", "15", "--json"]);
    assert_eq!(out.status.code(), Some(2));
    let envelope = envelope(&out);
    assert_eq!(
        envelope["error"]["exits"],
        serde_json::json!(["atc explain --list", "atc brief 15"])
    );
}
...
fn bare_explain_refuses_with_exit_2() {
    let dir = tempfile::TempDir::new().unwrap();
    let out = atc(dir.path(), &["explain"]);
    assert_eq!(out.status.code(), Some(2));
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(stderr.contains("an id, or --list"), "{stderr}");
    assert!(stderr.contains("atc explain --list"), "{stderr}");

    let out = atc(dir.path(), &["explain", "--json"]);
    assert_eq!(out.status.code(), Some(2));
    let envelope = envelope(&out);
    assert_eq!(
        envelope["error"]["id"],
...
/// gains `atc explain <id>` on both failure surfaces — the human
