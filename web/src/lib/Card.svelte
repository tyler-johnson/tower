<script lang="ts">
  ...
  let cells = $derived(
    show.filter((f) => f !== field).map((f) => ({ field: f, cell: cell(f, view, refs, now) })),
  );
  ...
  let rest = $derived(
    cells.filter((c) => !head.includes(c) && c !== age && c !== subject && !empty(c.cell)),
  );
...
<a
  ...
  ondragstart={start}
  ondragend={() => ondragend()}
  class="rounded-box border-base-300 bg-base-100 hover:bg-base-200 flex flex-col gap-1 border p-2 {open
    ? 'bg-base-200'
    ...
    <div class="flex items-baseline gap-2">
      {#each head as c (c.field)}
        ...
        {:else if c.cell.kind === "ref"}
          <span class="text-primary font-mono">{c.cell.text}</span>
        {:else if c.cell.kind === "dot"}
          <span class="status {statusDot(c.cell.status)}" title={c.cell.status}></span>
        {/if}
      {/each}
      {#if age !== null && age.cell.kind === "dim"}
        <span class="text-base-content/40 ml-auto text-sm" title={fieldLabel("age")}>
          {age.cell.text}
        </span>
      {/if}
    </div>
