...
use atc_core::board::{Board, FlightView};
...
pub fn board(board: &Board, now: i64, colored: bool) -> String {
    ...
    if !board.unrouted.is_empty() {
        ...
        out.push_str(&paint_warn(
            &format!(
                "{} {noun} in the log {verb} not on the board — `atc doctor` says which and why",
                board.unrouted.len()
            ),
            colored,
        ));
    ...
    let footer = if flights == 0 {
        "nothing on the board · atc file to add one".to_string()
    } else {
        let noun = if flights == 1 { "flight" } else { "flights" };
        format!("{flights} {noun} · atc file to add one")
    };
...
mod tests {
    use super::*;
    use atc_core::board::WaitingOnYou;
    ...
    fn a_parent_prints_its_mark_and_a_sub_flight_its_own_subject() {
        ...
        assert!(out.contains("2 flights · atc file to add one"), "{out}");
    }
}
