...
//! test parses every verb's descriptor back through clap the way fufu
//! will — every verb, not only the served ones, so promoting a verb to
//! a tool is a list edit and never a schema surprise — and drift is a
//! failing test, not a policy. fufu spells a call from the
...
mod tests {
    ...
    /// Every option on every verb has a spelling fufu can emit: a long,
    ...
    /// Every verb's descriptor, spelled the way fufu spells a call,
