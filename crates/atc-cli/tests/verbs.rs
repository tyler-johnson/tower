...
fn brief_of(repo: &Path, flight: &str) -> serde_json::Value {
    envelope(&atc(repo, &["brief", flight, "--json"]))["data"].clone()
}
...
#[test]
fn a_bare_file_echoes_the_ready_filing_and_the_tail() {
    let repo = repo();
    let out = stdout(&atc(repo.path(), &["file", "fix the flaky retry test"]));
    assert_eq!(
        out,
        "filed #1 in ready: fix the flaky retry test\nboard: atc\n"
    );
}
...
#[test]
fn a_bare_file_is_one_ready_flight_and_one_event() {
    let repo = repo();
    let out = atc(repo.path(), &["file", "a plain one", "--json"]);
    ...
    assert_eq!(
        ...
        "born Ready: the default clears it at once"
    );
...
fn file_under_review_echoes_the_parent_and_its_flights() {
    let repo = repo();
    install_review(&repo);
    let out = stdout(&atc(repo.path(), &["file", "review", "the retry test"]));
    assert_eq!(
        out,
...
fn a_procedure_that_is_not_installed_is_refused() {
    let repo = repo();
    install_chore(&repo);
    let out = atc(repo.path(), &["file", "ghost", "a subject", "--json"]);
    ...
    assert_eq!(
        refused["error"]["exits"],
        serde_json::json!(["atc procedures"])
    );
...
#[test]
fn a_filed_flight_lands_in_the_ready_group() {
    let repo = repo();
    stdout(&atc(repo.path(), &["file", "land on the board"]));
    let board = envelope(&atc(repo.path(), &["--json"]));
    assert_eq!(board["data"]["ready"][0]["id"], serde_json::json!("pi.1"));
    assert_eq!(
        board["data"]["ready"][0]["subject"],
...
fn an_empty_subject_is_a_usage_refusal() {
    let repo = repo();
    let out = atc(repo.path(), &["file", "   ", "--json"]);
...
fn an_empty_procedure_is_a_usage_refusal() {
    let repo = repo();
    let out = atc(repo.path(), &["file", "", "a subject", "--json"]);
...
fn the_setting_parks_a_bare_filing_and_the_flag_beats_it() {
    ...
    let out = stdout(&atc(repo.path(), &["file", "parked"]));
    ...
    // Back to the default: the next filing is born Ready again.
    stdout(&atc(
        repo.path(),
        &["config", "--unset", "defaultFileStatus"],
    ));
...
#[test]
fn an_unfileable_status_at_filing_is_a_usage_refusal() {
    let repo = repo();
    for word in ["held", "waiting", "done", "canceled", "claimed"] {
        let out = atc(repo.path(), &["file", "no", "--status", word, "--json"]);
        let envelope = refusal(&out, 2, "usage/file-status");
        assert_eq!(
            envelope["error"]["message"],
...
#[test]
fn a_procedure_flight_declaring_a_status_keeps_it_beside_the_default() {
    let repo = repo();
    repo.write(
    ...
    stdout(&atc(repo.path(), &["file", "staged", "the thing"]));
    ...
    assert_eq!(
        board["data"]["ready"][0]["subject"],
        serde_json::json!("the thing · do"),
        "undeclared, the default"
    );
    assert_eq!(
        board["data"]["waiting"][0]["subject"],
        serde_json::json!("the thing"),
        "the parent waits on both"
    );
}
...
fn a_comment_counts_on_the_board() {
    ...
    let out = stdout(&atc(repo.path(), &["comment", "pi.1", "-m", "a note"]));
    assert_eq!(out, "commented on #1\nboard: atc\n");
    let board = envelope(&atc(repo.path(), &["--json"]));
    assert_eq!(board["data"]["ready"][0]["comments"], serde_json::json!(1));
}
...
fn a_comment_on_a_missing_flight_is_refused() {
    let repo = repo();
    let out = atc(repo.path(), &["comment", "pi.99", "-m", "x", "--json"]);
    let envelope = refusal(&out, 1, "flight/not-found");
    assert_eq!(envelope["error"]["exits"], serde_json::json!(["atc"]));

    // The human rendering of the same refusal: stderr, with the try block.
    let out = atc(repo.path(), &["comment", "pi.99", "-m", "x"]);
    assert_eq!(out.status.code(), Some(1));
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(stderr.starts_with("atc: "), "got {stderr:?}");
...
fn a_comment_without_a_note_is_refused() {
    let repo = repo();
    let out = atc(repo.path(), &["comment", "pi.1", "--json"]);
    let envelope = refusal(&out, 2, "usage/needs-message");
    assert_eq!(
        envelope["error"]["exits"],
        serde_json::json!(["atc comment <flight> -m <note>"])
    );
}
...
fn a_self_link_is_a_usage_refusal() {
    let repo = repo();
    stdout(&atc(repo.path(), &["file", "itself"]));
    let out = atc(repo.path(), &["link", "pi.1", "pi.1", "--json"]);
    ...
    let out = atc(repo.path(), &["link", "1", "1", "--json"]);
    refusal(&out, 2, "usage/self-link");
}
...
fn a_link_to_a_missing_flight_is_refused() {
    ...
    let out = atc(repo.path(), &["link", "pi.1", "pi.9", "--json"]);
...
fn an_unlink_to_a_missing_flight_is_refused() {
    let repo = repo();
    stdout(&atc(repo.path(), &["file", "the only flight"]));
    let out = atc(repo.path(), &["unlink", "pi.1", "pi.9", "--json"]);
    refusal(&out, 1, "flight/not-found");
}
...
fn a_comment_shows_in_the_rendered_note_line() {
    let repo = repo();
    stdout(&atc(repo.path(), &["file", "carry a note"]));
    stdout(&atc(repo.path(), &["comment", "pi.1", "-m", "a note"]));
...
fn a_bare_seq_resolves_against_the_board() {
    let repo = repo();
    stdout(&atc(repo.path(), &["file", "take a bare seq"]));
    stdout(&atc(repo.path(), &["comment", "1", "-m", "note"]));
    let board = envelope(&atc(repo.path(), &["--json"]));
    assert_eq!(board["data"]["ready"][0]["comments"], serde_json::json!(1));
}
...
fn a_hash_prefixed_reference_is_accepted() {
    let repo = repo();
    stdout(&atc(repo.path(), &["file", "take a pasted ref"]));
    stdout(&atc(repo.path(), &["comment", "#1", "-m", "note"]));
    let board = envelope(&atc(repo.path(), &["--json"]));
    assert_eq!(board["data"]["ready"][0]["comments"], serde_json::json!(1));
}
...
fn a_bare_seq_with_no_match_is_not_found() {
    let repo = repo();
    stdout(&atc(repo.path(), &["file", "the only flight"]));
    let out = atc(repo.path(), &["comment", "9", "-m", "x", "--json"]);
    refusal(&out, 1, "flight/not-found");
}
...
fn a_bare_seq_links_and_the_wire_stays_full() {
    ...
    let board = envelope(&atc(repo.path(), &["--json"]));
    // Born Ready, so the edge folds it Waiting.
...
fn a_status_move_puts_the_flight_in_progress_with_the_pilots_byline() {
    ...
    let out = stdout(&atc(repo.path(), &["status", "1", "in_progress"]));
    assert_eq!(out, "moved #1 to in progress: take a pull\nboard: atc\n");
...
fn status_json_carries_the_appended_move() {
    let repo = repo();
    stdout(&atc(repo.path(), &["file", "moved work"]));
...
fn a_word_outside_the_status_vocabulary_is_refused() {
    let repo = repo();
    stdout(&atc(repo.path(), &["file", "standing work"]));
    let out = atc(repo.path(), &["status", "1", "claimed", "--json"]);
...
fn waiting_and_held_are_refused_with_the_verb_that_derives_them() {
    ...
    let out = atc(repo.path(), &["status", "1", "waiting", "--json"]);
    let refused = refusal(&out, 2, "usage/status-waiting");
    assert_eq!(
        refused["error"]["message"],
        serde_json::json!(
            "`#1` cannot be set waiting — waiting comes from links: `atc link <flight> <dependency>`"
        )
    );
    assert_eq!(
        refused["error"]["exits"],
        serde_json::json!(["atc link <flight> <dependency>"])
    );

    let out = atc(repo.path(), &["status", "1", "held", "--json"]);
    let refused = refusal(&out, 2, "usage/status-held");
    assert_eq!(
        refused["error"]["message"],
        serde_json::json!(
            "`#1` cannot be set held — held comes from a question: `atc hold <flight> -m <question>`"
        )
    );
    assert_eq!(
        refused["error"]["exits"],
        serde_json::json!(["atc hold <flight> -m <question>"])
    );

    // Nothing landed: the board still holds one Ready flight.
    let board = envelope(&atc(repo.path(), &["--json"]));
    assert_eq!(
        board["data"]["ready"][0]["status"],
        serde_json::json!("ready")
    );
}
...
fn ready_on_a_gated_flight_lands_waiting_and_the_echo_says_so() {
    ...
    let out = stdout(&atc(repo.path(), &["status", "1", "ready"]));
    assert_eq!(
        out,
    ...
    stdout(&atc(repo.path(), &["cancel", "2", "-m", "not needed"]));
    let board = envelope(&atc(repo.path(), &["--json"]));
...
fn assign_moves_the_lane_and_none_clears_it() {
    ...
    let out = stdout(&atc(repo.path(), &["assign", "1", "agent"]));
    ...
    let out = stdout(&atc(repo.path(), &["assign", "1", "none"]));
    assert_eq!(out, "cleared the lane on #1: laned work\nboard: atc\n");
    let board = envelope(&atc(repo.path(), &["--json"]));
    assert!(board["data"]["ready"][0]["assignee"].is_null());
}
...
fn hold_echoes_on_stdout_and_still_exits_three() {
    let repo = repo();
    stdout(&atc(repo.path(), &["file", "stuck"]));
    let out = atc(repo.path(), &["hold", "1", "-m", "which retry path?"]);
...
fn a_hold_without_a_question_is_refused() {
    let repo = repo();
    stdout(&atc(repo.path(), &["file", "stuck"]));
    let out = atc(repo.path(), &["hold", "1", "--json"]);
    let envelope = refusal(&out, 2, "usage/needs-message");
    assert_eq!(
        envelope["error"]["message"],
...
fn a_second_hold_is_refused_quoting_the_open_question() {
    let repo = repo();
    stdout(&atc(repo.path(), &["file", "stuck"]));
    atc(repo.path(), &["hold", "1", "-m", "which retry path?"]);
    let out = atc(repo.path(), &["hold", "1", "-m", "another?", "--json"]);
    ...
    assert_eq!(
        envelope["error"]["exits"],
        serde_json::json!(["atc answer <flight> -m <answer>"])
    );
}
...
fn answer_releases_the_hold_to_ready() {
    ...
    stdout(&atc(repo.path(), &["status", "1", "in_progress"]));
    atc(repo.path(), &["hold", "1", "-m", "which retry path?"]);
    let out = stdout(&atc(repo.path(), &["answer", "1", "-m", "the outer one"]));
...
fn answer_on_a_gated_flight_lands_waiting() {
    let repo = repo();
    stdout(&atc(repo.path(), &["file", "stuck"]));
    stdout(&atc(repo.path(), &["file", "its dependency"]));
    stdout(&atc(repo.path(), &["link", "1", "2"]));
    stdout(&atc(repo.path(), &["status", "1", "in_progress"]));
    atc(repo.path(), &["hold", "1", "-m", "which retry path?"]);
    stdout(&atc(repo.path(), &["answer", "1", "-m", "the outer one"]));
...
fn an_answer_with_no_open_question_is_refused() {
    let repo = repo();
    stdout(&atc(repo.path(), &["file", "never held"]));
    let out = atc(repo.path(), &["answer", "1", "-m", "to what?", "--json"]);
...
fn done_takes_the_flight_off_the_board_and_out_of_the_count() {
    ...
    let out = stdout(&atc(repo.path(), &["done", "1"]));
...
fn cancel_closes_with_the_reason_on_the_move() {
    let repo = repo();
    stdout(&atc(repo.path(), &["file", "abandoned"]));
    let out = stdout(&atc(
        repo.path(),
    ...
    let out = atc(repo.path(), &["cancel", "1", "--json"]);
...
fn answering_a_closed_flight_that_was_held_names_the_abandoned_question() {
    let repo = repo();
    stdout(&atc(repo.path(), &["file", "stuck"]));
    atc(repo.path(), &["hold", "1", "-m", "which?"]);
    stdout(&atc(repo.path(), &["cancel", "1", "-m", "superseded"]));

    let out = atc(repo.path(), &["answer", "1", "-m", "a", "--json"]);
    let envelope = refusal(&out, 1, "flight/done");
    assert_eq!(
        envelope["error"]["message"],
...
fn the_lifecycle_verbs_refuse_a_closed_flight_but_a_comment_lands() {
    ...
    let out = atc(repo.path(), &["assign", "1", "agent", "--json"]);
    refusal(&out, 1, "flight/done");
    let out = atc(repo.path(), &["hold", "1", "-m", "q?", "--json"]);
    refusal(&out, 1, "flight/done");
    let out = atc(repo.path(), &["answer", "1", "-m", "a", "--json"]);
    refusal(&out, 1, "flight/done");

    // A note on the record is fine — only the lifecycle verbs refuse.
    stdout(&atc(repo.path(), &["comment", "1", "-m", "postscript"]));
}
...
fn decompose_json_carries_the_parent_the_children_and_the_edges() {
    let repo = repo();
    install_chore(&repo);
    stdout(&atc(repo.path(), &["file", "chore", "a broad task"]));
    let out = atc(
        repo.path(),
...
fn decompose_under_a_procedure_mints_the_definitions_flights() {
    let repo = repo();
    install_review(&repo);
    stdout(&atc(repo.path(), &["file", "look this over"]));
    let out = stdout(&atc(repo.path(), &["decompose", "1", "review"]));
    assert!(
    ...
    // re-stamps — and the edges fold the Ready-born parent Waiting.
    assert_eq!(parent["status"], serde_json::json!("waiting"));
}
...
fn decompose_with_nothing_to_split_into_is_a_usage_refusal() {
    let repo = repo();
    stdout(&atc(repo.path(), &["file", "a broad task"]));
    let out = atc(repo.path(), &["decompose", "1", "--json"]);
...
fn a_sub_flight_trimmed_to_nothing_is_a_usage_refusal() {
    let repo = repo();
    stdout(&atc(repo.path(), &["file", "a broad task"]));
    let out = atc(repo.path(), &["decompose", "1", "part one", "  ", "--json"]);
    refusal(&out, 2, "usage/empty-subject");
...
fn decomposing_a_closed_flight_is_refused() {
    ...
    stdout(&atc(repo.path(), &["file", "finished"]));
    stdout(&atc(repo.path(), &["done", "1"]));
    let out = atc(repo.path(), &["decompose", "1", "too late", "--json"]);
    refusal(&out, 1, "flight/done");
}
