...
//! `FF_NONINTERACTIVE=1`, and `ATC_FF=/nonexistent` — an answer that
...
fn the_manifest_declares_tower() {
    ...
    let v = reply(&atc(dir.path(), &["--ff-manifest"]));
...
fn a_skill_the_binary_does_not_ship_is_refused() {
    let dir = tempfile::TempDir::new().unwrap();
    for name in ["tower-loop", "tower-nothing"] {
        let out = atc(dir.path(), &["--ff-skill", name]);
        ...
        assert_eq!(
            v["error"]["exits"],
            serde_json::json!(["atc explain tower/skill/unknown"])
        );
    }
}
...
fn the_tools_are_the_loops_four_gestures() {
    let dir = tempfile::TempDir::new().unwrap();
    let v = reply(&atc(dir.path(), &["--ff-tools"]));
...
fn the_tower_skill_is_the_embedded_manual() {
    let dir = tempfile::TempDir::new().unwrap();
    let v = reply(&atc(dir.path(), &["--ff-skill", "tower"]));
    assert_eq!(v["cmd"], "tower --ff-skill");
    assert_eq!(
...
fn a_handshake_flag_behind_a_verb_is_claps_refusal() {
    let dir = tempfile::TempDir::new().unwrap();
    let out = atc(dir.path(), &["board", "--ff-manifest"]);
